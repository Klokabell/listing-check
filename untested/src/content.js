...
const renderDisplay_1 = require("./utilities/renderDisplay");
const processListingText_1 = require("./utilities/processListingText");
const companyNameSelector = ".job-details-jobs-unified-top-card__company-name";
const jobTitleSelector = ".job-details-jobs-unified-top-card__container--two-pane h1";
const locationSelector = ".job-details-jobs-unified-top-card__primary-description-container > div > span:nth-child(1)";
const resultsObject = {
    company: "",
    title: "",
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.action === "extractedValues") {
        if (message.values) {
            ...
            container.innerHTML = (0, renderDisplay_1.renderDisplay)(resultsObject);
        }
    }
});
const observer = new MutationObserver(function (_mutationlist) {
    ...
    const listingElementsArray = document.getElementsByClassName("jobs-details__main-content jobs-details__main-content--single-pane full-width");
    ...
    if (listingElementsArray.length > 0) {
        for (const mutation of _mutationlist) {
            if (mutation.type === "characterData") {
                ...
                const listingText = (0, processListingText_1.extractComponentText)(listingElement);
                if (companyDiv && listingText && jobTitle && locationElement) {
                    ...
                    (0, messageSender_1.messageSender)(listingText, userLanguage);
                }
                else {
                    console.log("resultsObject", resultsObject);
                }
